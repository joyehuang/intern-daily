...
- [ ] **P0 - 基础架构**：
  - 清理现有代码（移除"次日计划"、"杠杆"改"含金量"）
  - 设计 SQLite schema（3 张表：daily_reports, code_contexts, user_profile）
  - 实现配置管理（交互式 `init` + 手动编辑）

- [ ] **P0 - 日报生成**：
  - Git 提交解析（提取 ±10 行上下文）
  - 代码语法高亮（Shiki）
  - 技能标签分类（保留现有逻辑）
  - **AI 含金量评估**（结合历史对比，14 天窗口）
  - AI 总结（OpenAI GPT-4o-mini）
  - Markdown 输出（带语法高亮代码块）
  - 存入 SQLite

- [ ] **P0 - 记忆功能**：
  - 从 SQLite 读取历史 14 天数据
  - 构建 AI Prompt（用户背景 + 历史摘要 + 今日详情）
  - AI 输出 JSON：`{ level, reason, assessment }`
...
1. **清理现有代码**：
   - 移除 `develop.md` 中的旧逻辑（次日计划、杠杆概念）
   - 重构 `types.ts`：新增含金量相关字段
   - 更新 CLI 参数（移除 `--no-ai`，AI 是核心功能）

2. **设计数据库**：
   - 编写 SQLite migration
   - 实现 DB 工具函数（CRUD）

3. **实现 `intern-daily init`**：
   - 交互式问答收集用户信息
   - 生成 `.intern-daily.config.json`
